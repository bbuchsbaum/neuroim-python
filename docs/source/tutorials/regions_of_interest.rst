...
In neuroimpy there is comprehensive support for creating and working with regions of interest (ROI).
...
To create a spherical region of interest with a 5mm radius around a central voxel at (20, 20, 20)::
    ...
    vol = neuroimpy.read_vol("brain_image.nii.gz")
    
    # Create a spherical ROI centered at voxel [20, 20, 20] with 5mm radius
    sphere = neuroimpy.spherical_roi(vol, [20, 20, 20], radius=5, fill=100)
...
Suppose our real-world coordinate is at (-34, -28, 10) in MNI space::
    ...
    sphere = neuroimpy.spherical_roi(vol, vox, radius=10, fill=1)
...
We may want to convert a region of interest to a NeuroVol instance without storing every voxel value::
    ...
    sphere = neuroimpy.spherical_roi(vol, [50, 10, 10], radius=10, fill=1)
    print(sphere)
...
Create a square ROI in a specific plane::
    ...
    square = neuroimpy.square_roi(vol, centroid=[30, 30, 10], 
...
Create a 3D box ROI::
    ...
    cube = neuroimpy.cuboid_roi(vol, centroid=[30, 30, 15], 
                               surround=5, fill=1)
    
    # Create an asymmetric cuboid
    cuboid = neuroimpy.cuboid_roi(vol, centroid=[30, 30, 15], 
...
Create multiple spherical ROIs efficiently::
    ...
    roi_list = neuroimpy.spherical_roi_set(vol, centers, radius=6, 
                                          fill=[100, 200, 300])
...
A randomized searchlight samples voxels without replacement::
    ...
    from neuroimpy import random_searchlight
...
Use a parcellation to define ROIs::
    ...
    kvol = neuroimpy.ClusteredNeuroVol(cluster_data, vol.space)
    
    # Create ROIs from clusters
    from neuroimpy import clustered_searchlight
...
Extract time series from 4D data using ROIs::
    ...
    # Load 4D data
    vec = neuroimpy.read_vec("functional_data.nii")
    
    # Create ROI
    roi = neuroimpy.spherical_roi(vec, [32, 32, 16], radius=10)
...
ROIs can be saved as sparse volumes::
    ...
    # Create ROI
    roi = neuroimpy.spherical_roi(vol, [30, 30, 15], radius=8)
    
    # Convert to sparse volume and save
    sparse_roi = roi.as_sparse()
    neuroimpy.write_vol(sparse_roi, "my_roi.nii.gz")
    
    # Load it back
    loaded_roi = neuroimpy.read_vol("my_roi.nii.gz")
    ...
    reconstructed_roi = neuroimpy.ROIVol(roi_values, loaded_roi.space, roi_coords)