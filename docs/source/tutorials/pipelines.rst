...
First, let's find connected components in a thresholded volume::
    ...
    vol = neuroimpy.read_vol("brain_mask.nii")
    ...
    comp = neuroimpy.conn_comp(vol2, threshold=0.8)
...
Compute the local standard deviation within a spherical neighborhood::
    ...
    sd_vol = neuroimpy.SparseNeuroVol(
...
Replace each voxel with the average of its k-nearest neighbors::
    ...
    smoothed_vol = neuroimpy.SparseNeuroVol(
...
For efficiency, sometimes we only need coordinates without creating full ROI objects::
    ...
    avg_vol = neuroimpy.SparseNeuroVol(
...
Apply operations to each volume in a NeuroVec::
    ...
    vec = neuroimpy.neurovec([vol, vol, vol, vol, vol])
...
Apply operations to each voxel's time series::
    ...
    vec = neuroimpy.neurovec(np.random.randn(10, 10, 10, 50))
    
    # Method 1: Using numpy operations
    mean_vol_data = np.mean(vec.data, axis=3)
    mean_vol = neuroimpy.DenseNeuroVol(mean_vol_data, 
                                       neuroimpy.NeuroSpace(dim=mean_vol_data.shape))
    ...
    mean_vol2 = neuroimpy.DenseNeuroVol(mean_vol2_data, mean_vol.space)
...
Combine operations into reusable pipelines::
    ...
    def denoise_pipeline(vol, smooth_radius=5, threshold=0.1):
        ...
        comp = neuroimpy.conn_comp(vol, threshold=threshold)
        ...
        result = neuroimpy.DenseNeuroVol(np.zeros_like(vol.data), vol.space)
...
Create custom iterators for specific processing patterns::
    ...
    # Use the iterator
    vec = neuroimpy.neurovec(np.random.randn(10, 10, 10, 100))
