...
3. **Two-lane parity.** Legacy surfaces (`neuroim.compat`, R-shaped APIs) are hard-gated against neuroim2 byte-for-byte. New Python-native return objects are gated on their numeric projection (`result.values`, `result.map_to_volume()`), not their schema.
