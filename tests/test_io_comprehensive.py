...
class TestReadWriteVol:
    ...
    def test_write_vol_accepts_float_alias(self):
        ...
        vol = DenseNeuroVol(np.ones((3, 3, 3), dtype=np.float32), NeuroSpace(dim=(3, 3, 3)))
    ...
    def test_write_vol_accepts_nifti_format_aliases(self):
        ...
        vol = DenseNeuroVol(np.ones((4, 4, 4), dtype=np.float32), NeuroSpace(dim=(4, 4, 4)))
...
class TestReadWriteVec:
    ...
    def test_read_vec_with_indices(self):
        ...
        try:
            ...
            for i, idx in enumerate(indices):
                np.testing.assert_array_equal(vec.data[:, :, :, i], data_4d[:, :, :, idx])
    ...
    def test_write_vec_defaults_to_float_type(self):
        """Test no data_type uses default float output."""
        vec = DenseNeuroVec(np.arange(24, dtype=np.int16).reshape((4, 3, 2, 1)), NeuroSpace(dim=(4, 3, 2, 1)))
