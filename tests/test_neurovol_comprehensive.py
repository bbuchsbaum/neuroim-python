...
from neuroim import (NeuroSpace, DenseNeuroVol, SparseNeuroVol, LogicalNeuroVol, write_vol)
...
class TestNeuroVolIndexing:
    ...
    def test_coordinate_indexing(self):
        ...
        expected = np.array([self.dat[0, 0, 0], self.dat[10, 10, 10], self.dat[20, 20, 20]])
...
class TestNeuroVolMethods:
    ...
    def test_which_min_max(self):
        ...
        # Check values
        assert self.vol[min_coords[0, 0], min_coords[0, 1], min_coords[0, 2]] == self.vol.min()
        assert self.vol[max_coords[0, 0], max_coords[0, 1], max_coords[0, 2]] == self.vol.max()
