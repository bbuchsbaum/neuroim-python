...
| 20 | **Parcellated connectome** — atlas → per-parcel mean series → `N x N` Pearson correlation matrix | **safety + inspectability win, ties on lines** — the bare-array path is affine-blind, so a wrong-frame atlas yields a byte-identical (silently wrong) connectome; `parcel_means` gates it and threads provenance | 2 — both **P3, open**: no typed `connectome()` reducer; the `(n_time, n_parcels)` matrix accessor (`.ts`) is semi-public. See `20_parcellated_connectome/REPORT.md`. |
