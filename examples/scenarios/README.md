...
| 15 | **Block parcel contrast** — compute task-minus-rest effects per atlas parcel, then map parcel effects back into a 3-D image | **clear readability + spatial-safety win** | 1 — **PAIN-1 (P2) closed**: `ClusteredNeuroVec.contrast()` now returns a typed `ParcelEffectResult` with `map_to_volume()`. See `15_block_parcel_contrast/REPORT.md`. |
| 16 | **Smoothed temporal-SNR QC map** — compute masked tSNR, smooth at FWHM in mm, and summarize the QC map | **safety + readability + inspectability win** | 1 — **PAIN-1 (P1) closed**: `gaussian_blur` now chains upstream `temporal_snr` provenance. See `16_smoothed_tsnr_qc/REPORT.md`. |
| 17 | **Local block-effect searchlight** — compute a spherical task-minus-rest effect map and project center values back to volume | **typed-result + spatial-safety win** | none — `SearchlightResult` carries centers, radius, method name, Receipt, and an owned `map_to_volume()` projection. See `17_searchlight_block_effect/REPORT.md`. |
| 19 | **Group-mean tSNR across subjects in template space** — N subjects → resample → tSNR → group mean | **per-subject win, group-reduce gap** — first scenario whose input is N typed maps and whose output is one map with N upstream chains | 4 — **PAIN-1 (P1)**: no first-class `group_mean` reducer. **PAIN-2 (P2)**: same-space gate at the group reduce is user code. **PAIN-3 (P2)**: no public multi-input Receipt helper. **PAIN-4 (P1, closed)**: `write_vol`/`write_vec` now route through `to_nibabel` and embed the receipt. See `19_group_mean_template_tsnr/REPORT.md`. |
| 20 | **Parcellated connectome** — atlas → per-parcel mean series → `N x N` Pearson correlation matrix | **safety + inspectability + lines win** — the bare-array path is affine-blind, so a wrong-frame atlas yields a byte-identical (silently wrong) connectome; `parcel_means().connectome()` gates it and threads provenance | 2 — both **P3, closed**: typed `ClusteredNeuroVec.connectome()` reducer and public `timeseries_matrix()` accessor landed. See `20_parcellated_connectome/REPORT.md`. |
