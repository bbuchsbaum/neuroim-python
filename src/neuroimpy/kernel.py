...
Direct translation of R's neuroim2 Kernel class and related functionality.
"""
...
class Kernel:
    ...
    def __init__(self, width: Tuple[int, ...], weights: np.ndarray, 
                 voxels: np.ndarray, coords: np.ndarray):
...
def gaussian_kernel(vdim: Tuple[float, ...], kdim: Tuple[int, ...], 
                   sigma: float, normalize: bool = True) -> Kernel:
    ...
    coords = np.where(cvoxel_ind == 0, 0, np.sign(cvoxel_ind) * (np.abs(cvoxel_ind) - 0.5))
...
def spherical_kernel(vdim: Tuple[float, ...], radius: float, 
                    weight_func: Optional[Callable] = None) -> Kernel:
...
def embed_kernel(kernel: Kernel, space: NeuroSpace, center_voxel: np.ndarray,
                weight: float = 1.0) -> SparseNeuroVol:
