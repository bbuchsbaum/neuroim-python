...
class NamedAxis:
    ...
    Direct translation of R's NamedAxis class.
    ...
    neuroim2::NamedAxis
    """
    def __init__(self, axis: str, direction: Union[int, List[float], np.ndarray]):
        self.axis = axis
    ...
    def __eq__(self, other):
        if not isinstance(other, NamedAxis):
            return False
        return self.axis == other.axis and np.array_equal(self.direction, other.direction)
...
class AxisSet:
    ...
    Direct translation of R's AxisSet class.
...
class AxisSet5D(AxisSet):
    ...
    neuroim2::AxisSet5D
    """
    def __init__(self, i: NamedAxis, j: NamedAxis, k: NamedAxis, l: NamedAxis, m: NamedAxis):
        super().__init__(5)
        self.i = i
        self.j = j
        self.k = k
...
def axis_set(ndim: int, axes: List[Union[str, NamedAxis]] = None) -> Union[AxisSet1D, AxisSet2D, AxisSet3D, AxisSet4D, AxisSet5D, AxisSetND]:
    if axes is None:
        axes = ["x", "y", "z", "t", "v"][:ndim]
        if ndim > 5:
    ...
    if ndim == 1:
        ...
        return AxisSet5D(named_axes[0], named_axes[1], named_axes[2], named_axes[3], named_axes[4])
    else:
        return AxisSetND(named_axes)

def axis_names(x: Union[AxisSet1D, AxisSet2D, AxisSet3D, AxisSet4D, AxisSet5D, AxisSetND]) -> List[str]:
    if isinstance(x, AxisSetND):
        return [ax.axis for ax in x.axes]
    return [getattr(x, attr).axis for attr in "ijklm"[:x.ndim]]

def axis_directions(x: Union[AxisSet1D, AxisSet2D, AxisSet3D, AxisSet4D, AxisSet5D, AxisSetND]) -> List[int]:
    if isinstance(x, AxisSetND):
...
def permute_axes(x: Union[AxisSet1D, AxisSet2D, AxisSet3D, AxisSet4D, AxisSet5D, AxisSetND], perm: List[int]) -> Union[AxisSet1D, AxisSet2D, AxisSet3D, AxisSet4D, AxisSet5D, AxisSetND]:
    if len(perm) != x.ndim:
        raise ValueError(f"Length of permutation ({len(perm)}) must match ndim ({x.ndim})")
...
def drop_axis(x: Union[AxisSet2D, AxisSet3D, AxisSet4D, AxisSet5D, AxisSetND], which: Union[str, int]) -> Union[AxisSet1D, AxisSet2D, AxisSet3D, AxisSet4D, AxisSet5D, AxisSetND]:
    if isinstance(which, str):
        which = axis_names(x).index(which)
...
def add_axis(x: Union[AxisSet1D, AxisSet2D, AxisSet3D, AxisSet4D, AxisSet5D, AxisSetND], new_axis: str) -> Union[AxisSet1D, AxisSet2D, AxisSet3D, AxisSet4D, AxisSet5D, AxisSetND]:
    current_axes = list(x) if isinstance(x, AxisSetND) else [getattr(x, attr) for attr in "ijklm"[:x.ndim]]
...
def match_axis(axis_char: str) -> NamedAxis:
    ...
    neuroim2::matchAxis
    """
    normalized = str(axis_char).replace("-", "").replace("_", "").replace(" ", "").upper()
    full_to_short = {
        "LEFT": "L",
        "RIGHT": "R",
        "ANTERIOR": "A",
