...
Direct translation of R's neuroim2 binary I/O classes.
"""
...
class BinaryReader:
    ...
    def __init__(self, input_connection: Union[BinaryIO, str, Path],
                 byte_offset: int = 0, data_type: str = "float32",
                 bytes_per_element: int = 4, endian: str = "little",
                 signed: bool = True):
...
class BinaryWriter:
    ...
    def __init__(self, output_connection: Union[BinaryIO, str, Path],
                 byte_offset: int = 0, data_type: str = "float32",
                 bytes_per_element: int = 4, endian: str = "little"):
...
class ColumnReader:
    ...
    def __init__(self, input_connection: Union[BinaryIO, str, Path],
                 n_cols: int, n_rows: Optional[int] = None, **kwargs):
    ...
    def read_column(self, col_index: int) -> np.ndarray:
        ...
        if col_index < 0 or col_index >= self.n_cols:
            raise ValueError(f"Column index {col_index} out of range [0, {self.n_cols})")
        
        # Seek to column start
        offset = self.reader.byte_offset + col_index * self.n_rows * self.reader.bytes_per_element
    ...
    def read_all_columns(self) -> np.ndarray:
        ...
        if self.n_rows is None:
            ...
            if len(data) % self.n_cols != 0:
                raise ValueError(f"Data size {len(data)} not divisible by n_cols {self.n_cols}")
