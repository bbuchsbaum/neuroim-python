...
Direct translation of R's neuroim2 FileBackedNeuroVec class.
"""
...
class FileBackedNeuroVec(NeuroVec):
    ...
    def __init__(self, filenames: List[str], 
        ...
        self.dtype = first_vol.data.dtype
    ...
    def _load_volume(self, idx: int) -> np.ndarray:
        ...
        if vol.shape != self.vol_shape:
            raise ValueError(f"Volume {idx} has inconsistent shape: "
            ...
            if x.ndim == 2 and x.shape[1] == 3:
                ...
                for t in range(self.n_volumes):
                    ...
                    for i, coord in enumerate(x):
                        # Check bounds
                        if (0 <= coord[0] < self.vol_shape[0] and 
                            0 <= coord[1] < self.vol_shape[1] and 
                            0 <= coord[2] < self.vol_shape[2]):
...
def file_backed_neurovec(filenames: List[str], 
                        mask: Optional[np.ndarray] = None) -> FileBackedNeuroVec:
