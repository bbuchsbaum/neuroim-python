...
Lazy 4D vector container for datasets that are too large to keep fully in memory.
"""
...
class BigNeuroVec(NeuroVec):
    ...
    def __init__(
        self,
        data: Optional[Union[np.ndarray, np.memmap, str]] = None,
        ...
        if data is not None and space is not None:
            ...
            if data.shape != expected_shape:
                raise ValueError(
                    f"data shape {data.shape} does not match space dimensions {expected_shape}"
                )
            ...
            if (
                isinstance(data, np.memmap)
                and hasattr(data, "filename")
                and data.filename == self.filename
            ):
            ...
            space = NeuroSpace(dim=list(shape), spacing=[1.0] * 4, origin=[0.0] * 4)
            super().__init__(space)
            ...
            self._data = np.memmap(
                self.filename, dtype=dtype, mode="w+", shape=shape, order="C"
            )
            ...
            del self._data
            self._data = np.memmap(
                self.filename, dtype=dtype, mode=mode, shape=shape, order="C"
            )
        else:
            raise ValueError(
                "BigNeuroVec requires either (data, space) or (filename, shape)"
            )
    ...
    def sub_vector(self, indices: Union[slice, np.ndarray]) -> "BigNeuroVec":
        ...
        subset_vec = BigNeuroVec(
            subset_data,
            NeuroSpace(
                dim=subset_shape, spacing=self.space.spacing, origin=self.space.origin
            ),
        )
    ...
    def _arithmetic_op(self, other, op):
        ...
        if np.isscalar(other):
            result = BigNeuroVec(op(self._data, other), self.space)
        elif isinstance(other, BigNeuroVec):
            if self.shape != other.shape:
                raise ValueError("Incompatible shapes for arithmetic operation")
            result = BigNeuroVec(op(self._data, other._data), self.space)
        else:
...
def big_neurovecseq(vols: list, mask: Optional[np.ndarray] = None) -> BigNeuroVec:
    ...
    # Create BigNeuroVec
    vec = BigNeuroVec(
        np.zeros(vol_shape + (n_vols,), dtype=first_vol.data.dtype), space_4d
    )
