...
Direct translation of R's neuroim2 NeuroSlice class.
"""
...
class NeuroSlice:
    ...
    Direct translation of R's NeuroSlice class.
    ...
    def __init__(self, data: np.ndarray, space: NeuroSpace):
        ...
        if data.ndim == 1:
            ...
            if data.size != np.prod(space.dim):
                raise ValueError(f"Data length {data.size} must match space dimensions {np.prod(space.dim)}")
            data = data.reshape(tuple(space.dim), order='F')
        elif data.ndim == 2:
            if data.shape != tuple(space.dim):
                raise ValueError(f"Data shape {data.shape} must match space dimensions {tuple(space.dim)}")
        else:
    ...
    def _arithmetic_op(self, other, op):
        ...
        if isinstance(other, (int, float)):
            ...
            if other.shape == self.shape:
                ...
                raise ValueError(f"Array shape {other.shape} must match slice shape {self.shape}")
...
def neuroslice(data: Union[np.ndarray, list], space: NeuroSpace, indices: Optional[np.ndarray] = None) -> NeuroSlice:
